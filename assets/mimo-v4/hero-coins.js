...
import { GLTFLoader } from '../mimo-v3/vendor/loaders/GLTFLoader.js';

const COIN_ROOT_NAME = '_rig_animation_origin';
const HOVER_DELAY = 120;
const HOVER_DURATION = 1.15;
const TAU = Math.PI * 2;
const ORBIT_RADIUS = 3.75;
const COIN_SCALE = 0.88;
...
export function createHeroCoins({ root, modelUrl, onCoinHover } = {}) {
  ...
  try {
    ...
    ready = Promise.resolve()
      ...
      .then((gltf) => {
        ...
        coinNodes.forEach((coin, index) => {
          const angle = Math.PI + (index / coinNodes.length) * TAU;
          coin.position.x = Math.cos(angle) * ORBIT_RADIUS;
          coin.position.z = Math.sin(angle) * ORBIT_RADIUS;
          coin.scale.multiplyScalar(COIN_SCALE);
          coinGroup.add(coin);
        });
  ...
  const tick = (scrollProgress, time = performance.now() * 0.001) => {
    ...
    try {
      ...
      const scrollEase = easeInOutCubic(progress);
      const scaleLimit = reducedMotion ? 1.2 : 2.45;
      const cameraAdvance = reducedMotion ? 0.2 : (isMobile() ? 0.6 : 1.2);
